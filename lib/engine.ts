...
export function evaluateStrategy(strategy:keyof typeof SPECS,s:Snapshot){
 ...
 add('liquidity','مراجعة كفاية السيولة',s.liquidityReviewed===undefined?null:s.liquidityReviewed,'الحد العددي للارتداد غير محدد في المصدر؛ المراجعة مطلوبة.');
 ...
 const required=strategy==='bounce'?['price','marketCap','return12m','low52w','ma30w','dilution']:["price",'marketCap','medianDollarVolume20d','revenue',strategy==='core'?'evSales':'ps',...(finite(s.netIncome)&&s.netIncome>0?['netIncome']:['fcf'])];
