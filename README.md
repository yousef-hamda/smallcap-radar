...
- Shared versioned gate engine, Core/Legacy documented weights, and transparent scores from 100 with per-factor points and coverage. Bounce uses an explicit six-factor strict ranking from 100; it is diagnostic rather than a claimed probability. Every list entry must pass all configured eligibility gates; weighted factors and their missing evidence remain visible in the ranking and cannot turn UNKNOWN into PASS. No score can rescue a failed or UNKNOWN eligibility gate.
