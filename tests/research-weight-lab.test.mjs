...
const withEvidence=row=>{
 const ids=Object.entries(row.features??{}).filter(([,value])=>value!=null).map(([id])=>id);
 return {...row,availableAt:{...Object.fromEntries(ids.map(id=>[id,row.asOf])),...(row.availableAt??{})},periodEnd:{...Object.fromEntries(ids.map(id=>[id,row.asOf])),...(row.periodEnd??{})},sources:{...Object.fromEntries(ids.map(id=>[id,'SYNTHETIC TEST FIXTURE'])),...(row.sources??{})}};
};
...
test('weight lab blocks insufficient data instead of publishing invented weights',async()=>{
 ...
 await fs.writeFile(input,JSON.stringify(withEvidence({symbol:'A',asOf:'2024-01-01',features:{collapse:1,reversal:1,liquidity:1,dilution:1,offLow:1,size:1},safety:{tradable:'PASS',conflict:'PASS',criticalData:'PASS'},outcome:{label:1,observedAt:'2024-05-01'}}))+'\n');
...
test('weight lab rejects future evidence and duplicate observations',async()=>{
 ...
 const row=withEvidence({symbol:'A',asOf:'2024-01-01',features:{valuation:.5,quality:.5,shareDiscipline:.5,sizeCoverage:.5,growth:.5,insider:.5,marginTrend:.5,entry:.5,balance:.5},availableAt:{valuation:'2025-01-01'},safety:{tradable:'PASS',conflict:'PASS',criticalData:'PASS'},outcome:{label:0,observedAt:'2025-01-01'}});
...
test('weight lab keeps exact 100-point normalization and deterministic learned output',async()=>{
 ...
 for(let month=0;month<12;month++) for(let company=0;company<8;company++){
  ...
  rows.push(withEvidence({symbol:'S'+company,asOf,features:{collapse,reversal,liquidity:.8,dilution:.7,offLow:.6,size:.5},safety:{tradable:'PASS',conflict:'PASS',criticalData:'PASS'},outcome:{label,netUtility:label ? .2 : -.15,observedAt:new Date(Date.UTC(2023,month,25)).toISOString().slice(0,10)}}));
 }
...
test('unknown and failed safety states never enter the learning sample',async()=>{
 const dir=await fs.mkdtemp(path.join(os.tmpdir(),'radar-lab-')),input=path.join(dir,'bounce.jsonl');
 const base=withEvidence({symbol:'A',asOf:'2024-01-01',features:{collapse:.5,reversal:.5,liquidity:.5,dilution:.5,offLow:.5,size:.5},outcome:{label:1,observedAt:'2024-02-01'}});
...
test('firm holdout identity is stable across ticker aliases',async()=>{
 const dir=await fs.mkdtemp(path.join(os.tmpdir(),'radar-lab-')),input=path.join(dir,'bounce.jsonl');
 const row=(symbol,asOf,label)=>withEvidence({symbol,firmId:'CIK-42',asOf,features:{collapse:.5,reversal:.5,liquidity:.5,dilution:.5,offLow:.5,size:.5},safety:{tradable:'PASS',conflict:'PASS',criticalData:'PASS'},outcome:{label,observedAt:'2025-01-01'}});
