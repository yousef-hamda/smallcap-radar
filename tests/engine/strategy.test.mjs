...
test('Core thesis conditions are weighted and do not remove a row',()=>{const r=evaluateStrategy('core',{...base,confidence:'B',revenue:0});assert.equal(r.checks.find(c=>c.id==='revenue').role,'factor');assert.equal(r.checks.find(c=>c.id==='revenue').status,'FAIL');assert.equal(r.factorStatus,'FAIL');assert.equal(r.qualified,true);assert.equal(r.status,'PASS');assert(r.score>=0&&r.score<=100);assert.equal(r.screeningQualified,true);assert.equal(r.finalRanked,false)});
test('Core factor failures are visible while safety gates remain independent',()=>{const r=evaluateStrategy('core',{...base,confidence:'B',marketCap:2e9+1});assert.equal(r.checks.find(c=>c.id==='cap').role,'factor');assert.equal(r.checks.find(c=>c.id==='cap').status,'FAIL');assert.equal(r.factorStatus,'FAIL');assert.equal(r.status,'PASS');assert.equal(r.qualified,true);assert(r.score>=0&&r.score<=100)});
...
test('Bounce factors rank every safe row instead of acting as exclusion gates',()=>{const accepted=evaluateStrategy('bounce',base);assert(accepted.score>=0&&accepted.score<=100);assert.equal(accepted.scoreCoverage,100);assert.equal(accepted.factors.reduce((sum,f)=>sum+f.maxPoints,0),100);assert.equal(accepted.checks.find(c=>c.id==='reversal').role,'factor');assert.equal(accepted.screeningQualified,true);const weaker=evaluateStrategy('bounce',{...base,return12m:-.2});assert(weaker.score>=0&&weaker.score<=100);assert.equal(weaker.checks.find(c=>c.id==='collapse').role,'factor');assert.equal(weaker.screeningQualified,true);assert.equal(weaker.factorStatus,'FAIL');assert(weaker.score<accepted.score)});
...
test('UNKNOWN strategy factors lower confidence without becoming PASS',()=>{const r=evaluateStrategy('core',{...base,confidence:'B',sourceConflicts:[],deathSpiral:'unknown'});assert.equal(r.checks.find(c=>c.id==='deathSpiral').status,'UNKNOWN');assert.equal(r.factorStatus,'UNKNOWN');assert.equal(r.screeningQualified,true);assert.equal(r.status,'PASS');assert.equal(r.qualified,true);assert.equal(r.gateStatus,'PASS');assert.equal(r.measurableStatus,'UNKNOWN')});
