...
test('predictive conditions affect ranking but are not eligibility gates',()=>{const r=evaluateStrategy('core',{...base,confidence:'B',revenue:0});assert.equal(r.checks.find(c=>c.id==='revenue').role,'factor');assert.equal(r.factorStatus,'FAIL');assert.equal(r.qualified,true);assert.equal(r.status,'PASS');assert(r.score>=0&&r.score<=100);assert.equal(r.screeningQualified,true);assert.equal(r.finalRanked,false)});
...
test('Bounce exposes one deterministic ranking while predictive factors remain weighted',()=>{const accepted=evaluateStrategy('bounce',base);assert(accepted.score>=0&&accepted.score<=100);assert.equal(accepted.scoreCoverage,100);assert.equal(accepted.factors.reduce((sum,f)=>sum+f.maxPoints,0),100);const weaker=evaluateStrategy('bounce',{...base,return12m:-.2});assert(weaker.score>=0&&weaker.score<=100);assert.equal(weaker.checks.find(c=>c.id==='collapse').role,'factor');assert.equal(weaker.screeningQualified,true);assert(weaker.score<accepted.score)});
