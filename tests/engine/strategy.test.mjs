...
test('Core revenue/valuation/profitability gates are strict and cannot be rescued by score',()=>{const r=evaluateStrategy('core',{...base,confidence:'B',revenue:0});assert.equal(r.checks.find(c=>c.id==='revenue').role,'eligibility');assert.equal(r.factorStatus,'PASS');assert.equal(r.qualified,false);assert.equal(r.status,'FAIL');assert(r.score>=0&&r.score<=100);assert.equal(r.screeningQualified,false);assert.equal(r.finalRanked,false)});
test('failed eligibility gates cannot be rescued by a high score',()=>{const r=evaluateStrategy('core',{...base,confidence:'B',marketCap:2e9+1});assert.equal(r.checks.find(c=>c.id==='cap').role,'eligibility');assert.equal(r.status,'FAIL');assert.equal(r.qualified,false);assert(r.score>=0&&r.score<=100)});
...
test('Bounce entry gates remain strict while the 100-point score ranks survivors',()=>{const accepted=evaluateStrategy('bounce',base);assert(accepted.score>=0&&accepted.score<=100);assert.equal(accepted.scoreCoverage,100);assert.equal(accepted.factors.reduce((sum,f)=>sum+f.maxPoints,0),100);assert.equal(accepted.checks.find(c=>c.id==='reversal').role,'eligibility');assert.equal(accepted.screeningQualified,true);const weaker=evaluateStrategy('bounce',{...base,return12m:-.2});assert(weaker.score>=0&&weaker.score<=100);assert.equal(weaker.checks.find(c=>c.id==='collapse').role,'eligibility');assert.equal(weaker.screeningQualified,false);assert(weaker.score<accepted.score)});
...
test('Core exposes a strict completeness-adjusted 100-point diagnostic score',()=>{const r=evaluateStrategy('core',base);assert.equal(r.rawScore,63.1);assert.equal(r.evidenceScore,81.9);assert.equal(r.score,63.1);assert.equal(r.factors.reduce((sum,f)=>sum+f.maxPoints,0),100);assert(r.factors.every(f=>f.points>=0&&f.points<=f.maxPoints));assert.equal(r.scoreCoverage,77);const partial=evaluateStrategy('core',{...base,fcf:null});assert(partial.score<r.score)});
