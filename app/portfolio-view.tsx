...
function CompanyLogo({ symbol, size = 44, eager = false }: { symbol: string; size?: number; eager?: boolean }) {
  ...
  return <Image unoptimized className="company-logo" src={failedFor === symbol ? retrySrc : src} width={size} height={size} alt={`شعار ${symbol}`} loading={eager ? 'eager' : 'lazy'} onError={() => setFailedFor(symbol)} />;
}
...
export function squarifiedTreemap(positions: PortfolioPosition[]): TreemapNode[] {
  ...
  const layoutRoot = treemap<TreemapData>()
    ...
    .paddingOuter(0.8)
    .paddingInner(1.1)
    // A square-balanced ratio avoids unreadable slivers on narrow phone
    // screens while preserving each holding's exact proportional area.
