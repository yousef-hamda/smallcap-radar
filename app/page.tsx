...
const stageLabels:Record<number,string>={0:'تجهيز الجولة',1:'مزامنة الأسعار الجماعية',2:'تنظيف أنواع الأوراق',3:'فلترة الحجم والسيولة',4:'جمع SEC Frames',9:'التقييم المحلي',13:'حفظ النتائج'};
...
export default function RadarApp(){
 ...
 useEffect(()=>{if(!run||!['running','partial'].includes(run.status)||(run.offset>=run.total&&!run.retryPending))return;let tick=0;const timer=window.setInterval(async()=>{tick++;try{if(tick%3===0)await fetch('/api/scan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'step',runId:run.id})});await refresh()}catch{}},4000);return()=>window.clearInterval(timer)},[run]);
 ...
 async function enableNotifications(){
  ...
  try{
   ...
   setNotificationsEnabled(true);setNotificationState('ready');
   const testResponse=await fetch('/api/push/test',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({endpoint:subscription.endpoint})});const testPayload=await testResponse.json();if(!testResponse.ok)throw Error(testPayload.error||'تم الاشتراك لكن تعذّر إرسال إشعار الاختبار.');
   setNotice('تم التفعيل وإرسال إشعار اختبار إلى هذا الهاتف. سيصلك تنبيه آخر عند انتهاء الفحص.');
  }catch(e:any){if(typeof Notification!=='undefined'&&Notification.permission==='denied')setNotificationState('denied');setError(e.message||'تعذّر تفعيل الإشعارات.')}
 }
 ...
 async function openCompany(snapshot:Snapshot){setSelected(snapshot);if(demo)return;setDetailLoading(true);try{const response=await fetch(`/api/company?symbol=${encodeURIComponent(snapshot.symbol)}`);const payload=await response.json();if(!response.ok)throw Error(payload.error||'تعذّر تحميل التحقق العميق.');setSelected(payload.snapshot)}catch(e:any){setNotice(`تعذّر تحديث التفاصيل: ${e.message}`)}finally{setDetailLoading(false)}}
 ...
 const rows=useMemo(()=>evaluated.filter(({s,e})=>(!query||`${s.symbol} ${s.name}`.toLowerCase().includes(query.toLowerCase()))&&(tab!=='favorites'||favorites.includes(s.symbol))&&(filter==='all'||(filter==='PASS'?(e as any).screeningQualified:e.status===filter))).sort((a,b)=>{const rank={PASS:4,UNKNOWN:3,FAIL:1};return rank[b.e.status as keyof typeof rank]-rank[a.e.status as keyof typeof rank]||Number((b.e as any).screeningQualified)-Number((a.e as any).screeningQualified)||a.s.symbol.localeCompare(b.s.symbol)}),[evaluated,query,tab,favorites,filter]);
 ...
 return <div className="terminal-shell" dir="rtl">
  <aside className={`sidebar ${mobileNav?'open':''}`}>
   ...
   <div className="sidebar-status"><div className="status-line"><span className="live-dot"/><b>جاهز للفحص</b></div><p>النتائج محفوظة ويمكنك العودة إليها في أي وقت.</p><button className="install-link" onClick={()=>install?install.prompt():setInstallHelp(true)}><Download size={16}/> تثبيت التطبيق</button></div>
  </aside>
  {mobileNav&&<button className="nav-scrim" onClick={()=>setMobileNav(false)} aria-label="إغلاق القائمة"/>}

  <main className="app-main">
   <header className="appbar"><button className="menu-button" onClick={()=>setMobileNav(true)} aria-label="فتح القائمة"><Menu size={21}/></button><div className="breadcrumb"><span>رادار</span><ChevronLeft size={14}/><b>{tabTitles[tab]}</b></div><div className="appbar-meta"><span className="market-state"><span className="live-dot"/> البيانات متصلة</span><span className="avatar">YH</span></div></header>
   <div className="content">
    <section className="command-header"><div><span className="kicker">نظرة عامة</span><h1>{tabTitles[tab]}</h1><p>{tab==='home'?'اكتشف الشركات التي تستحق التحقق بوضوح وسرعة.':tab==='lab'?'اختبر القواعد والتكاليف قبل اعتماد أي نتيجة.':tab==='data'?'راقب تغطية كل مصدر وحالة البيانات.':'القرار أولًا، والتفاصيل متاحة عند الحاجة.'}</p></div><div className="scan-actions"><button className="scan-primary" onClick={()=>resumable?scan(run.source?.includes('quick')?'quick':'full'):setScanMenu(v=>!v)} disabled={busy}><RefreshCw size={17} className={busy||resumable?'spin':''}/>{busy?'جارٍ البدء':resumable?'الفحص يعمل':'فحص السوق'}</button>{scanMenu&&<div className="scan-popover"><button onClick={()=>scan('full')}><b>فحص السوق الكامل</b><span>كل السوق، ثم تحليل المرشحين فقط</span></button><button onClick={()=>scan('quick')}><b>فحص سريع</b><span>عينة من 12 سهمًا للتجربة</span></button></div>}</div></section>
    ...
    <div className={`overview-layout ${run?'has-run':''}`}>
     ...
     <section className="metric-row">
      ...
      <Metric icon={ShieldCheck} label="اجتازت الشروط" value={screeningQualified} meta="مراجعة نهائية" tone="positive"/>
      <Metric icon={Clock3} label="تحتاج تحقق" value={incomplete} meta="مراجعة" tone="warning"/>
      <Metric icon={Layers} label="مستبعدة" value={excluded} meta="خارج القائمة" tone="negative"/>
     </section>
    </div>

    {tab==='home'&&!notificationsEnabled&&<section className="notification-panel"><span className="notification-panel-icon"><Bell size={20}/></span><div><b>أخبرني عند اكتمال الفحص</b><p>{notificationState==='denied'?'الإشعارات محظورة. فعّلها من إعدادات الهاتف ثم افتح التطبيق مجددًا.':notificationState==='unsupported'?'على iPhone ثبّت التطبيق أولًا وافتحه من الشاشة الرئيسية.':'فعّل الإشعارات مرة واحدة، ويمكنك بعدها إغلاق التطبيق بأمان.'}</p></div><button onClick={notificationState==='unsupported'?()=>setInstallHelp(true):enableNotifications}>{notificationState==='unsupported'?'طريقة التثبيت':notificationState==='denied'?'التعليمات':'تفعيل الإشعارات'}</button></section>}
    {tab==='home'&&notificationsEnabled&&<section className="notification-panel enabled"><span className="notification-panel-icon"><Check size={20}/></span><div><b>الإشعارات مفعّلة</b><p>سنرسل لك تنبيهًا فور انتهاء الفحص.</p></div><button onClick={enableNotifications}>اختبار</button></section>}

    {tab==='lab'?<LabPanel lab={lab} setLab={setLab}/>:tab==='data'?<DataPanel run={run} exportData={exportData} importFile={importFile}/>:<>
     <section className="strategy-switch" aria-label="اختيار الاستراتيجية"><button className={strategy==='core'?'active':''} onClick={()=>{setStrategy('core');setFilter('all')}}><span><ShieldCheck size={18}/><b>القيمة الأساسية</b></span><small>استثمار 12–24 شهرًا</small></button><button className={strategy==='bounce'?'active':''} onClick={()=>{setStrategy('bounce');setFilter('all')}}><span><Activity size={18}/><b>فرص الارتداد</b></span><small>هدف قصير حتى 3 أشهر</small></button><div className="strategy-note"><Info size={16}/><span>{strategy==='core'?'تُعرض الشركات التي اجتازت الشروط الأساسية أو تحتاج تحققًا إضافيًا.':'هدف +20% ووقف −15% مع شروط سيولة معلنة.'}</span></div></section>

     <section className="research-panel"><div className="panel-heading"><div><h2>{tab==='favorites'?'قائمة المتابعة':'الشركات'} <em>{rows.length}</em></h2><p>الشروط الأساسية أولًا؛ الحالات المعلّقة تحتاج تحققًا إضافيًا.</p></div><button className="ghost-button" onClick={exportData}><Download size={16}/> تصدير</button></div><div className="control-row"><label className="search-field"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="ابحث بالاسم أو الرمز" aria-label="بحث الشركات"/></label><Tabs value={filter} onValueChange={setFilter} dir="rtl"><TabsList><TabsTrigger value="all">الكل</TabsTrigger><TabsTrigger value="PASS">اجتازت الشروط</TabsTrigger><TabsTrigger value="UNKNOWN">تحتاج تحقق</TabsTrigger><TabsTrigger value="FAIL">مستبعدة</TabsTrigger></TabsList></Tabs></div>
      {rows.length?<StockTable rows={rows} strategy={strategy} favorites={favorites} onSelect={openCompany} onFavorite={favorite}/>:<EmptyState hasData={!!data.length} onQuick={()=>scan('quick')} onDemo={()=>{setData(fixtures);setDemo(true);setRun(null);setError('');setNotice('')}}/>}
     </section>
    </>}
    <footer className="app-footer"><span>رادار الشركات الصغيرة V2</span><span>بيانات مؤرخة · مصادر معلنة · دون نتائج مصطنعة</span></footer>
   </div>
  </main>

  <nav className="bottom-nav" aria-label="التنقل الرئيسي">{navItems.slice(0,4).map(([id,name,Icon])=><button key={id} className={tab===id?'active':''} onClick={()=>switchTab(id)}><Icon size={20}/><span>{name.replace('لوحة الفرص','الفرص').replace('القيمة الأساسية','القيمة').replace('فرص الارتداد','الارتداد').replace('قائمة المتابعة','المتابعة')}</span></button>)}<button className={tab==='lab'||tab==='data'?'active':''} onClick={()=>setMobileNav(true)}><Menu size={20}/><span>المزيد</span></button></nav>

  <CompanyDialog selected={selected} strategy={strategy} loading={detailLoading} onClose={()=>setSelected(null)}/>
...
function StockTable({rows,strategy,favorites,onSelect,onFavorite}:{rows:any[];strategy:'core'|'bounce';favorites:string[];onSelect:(s:Snapshot)=>void;onFavorite:(symbol:string)=>void}){return <div className="table-wrap"><div className="table-head"><span>الشركة</span><span>السعر</span><span>القيمة السوقية</span><span>12 شهرًا</span><span>الدرجة</span><span>الحالة</span><span>جودة البيانات</span><span/></div>{rows.map(({s,e})=>{const screening=(e as any).screeningQualified;const badge=screening&&e.status!=='PASS'?'اجتاز الشروط':labels[e.status];return <div className="table-row" key={s.symbol}><button className="issuer" onClick={()=>onSelect(s)}><span className="ticker">{s.symbol.slice(0,2)}</span><span><b dir="ltr">{s.symbol}</b><small>{s.name}</small></span></button><button className="cell-number" onClick={()=>onSelect(s)}><b>${money(s.price)}</b></button><button className="cell-number" onClick={()=>onSelect(s)}><b>${money(s.marketCap)}</b></button><button className={`cell-number ${s.return12m!=null&&(s.return12m<0?'down':'up')}`} onClick={()=>onSelect(s)}><b>{pct(s.return12m)}</b></button><button className="cell-number score-cell" onClick={()=>onSelect(s)}><b>{strategy==='core'&&e.score!=null?`${e.score.toFixed(1)}`:'—'}</b><small>{strategy==='core'&&e.scoreCoverage<100?`${e.scoreCoverage}% بيانات`:'من 100'}</small></button><button className="status-cell" onClick={()=>onSelect(s)}><span className={`result-badge ${screening?'pass':e.status.toLowerCase()}`}>{badge}</span><small>{screening&&e.status!=='PASS'?'بانتظار التحقق النهائي':e.status==='PASS'?'كل الشروط مكتملة':e.checks.find((c:any)=>c.status===e.status)?.label||'راجع تفاصيل الفحص'}</small></button><button className="quality-cell" onClick={()=>onSelect(s)}><b>{s.confidence||'F'}</b><small>{strategy==='core'?'درجة قابلة للتفسير':'بوابات موثقة'}</small></button><button className={`watch ${favorites.includes(s.symbol)?'saved':''}`} aria-label="حفظ في قائمة المتابعة" onClick={()=>onFavorite(s.symbol)}><Star size={17}/></button></div>})}</div>}

function EmptyState({hasData,onQuick,onDemo}:{hasData:boolean;onQuick:()=>void;onDemo:()=>void}){return <div className="empty-state"><div className="empty-rule"/><span className="section-eyebrow">لا توجد سجلات مطابقة</span><h3>{hasData?'لا توجد نتائج ضمن هذا الفلتر':'لم تُنشأ لقطة سوق بعد'}</h3><p>{hasData?'عدّل البحث أو حالة النتائج.':'ابدأ بفحص سريع، وستظهر النتائج تباعًا مع حفظ التقدم.'}</p>{!hasData&&<div><button className="scan-primary" onClick={onQuick}>تشغيل فحص سريع</button><button className="text-button" onClick={onDemo}>بيانات توضيحية</button></div>}</div>}
...
function DataPanel({run,exportData,importFile}:{run:any;exportData:()=>void;importFile:(f?:File)=>void}){return <section className="research-panel content-panel"><div className="panel-heading"><div><span className="section-eyebrow">مصادر البيانات</span><h2>مركز البيانات</h2></div><button className="ghost-button" onClick={exportData}><Download size={16}/> سجل التدقيق</button></div>{run&&<div className="coverage-grid"><div><span>تغطية الأسعار</span><b>{run.quote_coverage?.toLocaleString('en-US')||0}</b><small>من {run.universe_total?.toLocaleString('en-US')||0}</small></div><div><span>مجموعات SEC Frames</span><b>{run.sec_success||0}/{run.sec_requests||0}</b><small>{run.sec_failed||0} غير متاحة</small></div><div><span>تغطية الأساسيات</span><b>{run.fundamental_coverage?.toLocaleString('en-US')||0}</b><small>شركة</small></div><div><span>المرحلة الحالية</span><b>{run.stage||0}</b><small>{stageLabels[run.stage]||'—'}</small></div></div>}<div className="source-grid"><article><div className="source-head"><span><Wifi size={18}/></span><div><b>Yahoo + Nasdaq</b><small>Bulk quotes · 250 رمزًا لكل طلب</small></div><span className="source-state"><span className="live-dot"/> جماعي</span></div><p>يُسعّر السوق في دفعات كبيرة، مع Nasdaq واللقطة المؤرخة كمسار احتياطي، بدل طلب تاريخ مستقل لكل شركة.</p></article><article><div className="source-head"><span><Server size={18}/></span><div><b>SEC EDGAR</b><small>XBRL Frames · 13 مجموعة رسمية</small></div><span className="source-state review">رسمي</span></div><p>تُجمع الأساسيات لآلاف الشركات حسب المقياس والفترة، مع لقطة رسمية مؤرخة عند حجب واجهة SEC للخادم. ينتقل Company Facts والتاريخ التفصيلي إلى التحقق عند فتح المرشح.</p></article></div><div className="data-actions"><div><h3>استيراد لقطة موثقة</h3><p>JSON من نوع Snapshot[] مع المصدر وتاريخ التوفر لكل مقياس.</p></div><label className="ghost-button upload"><FileUp size={16}/> اختيار ملف<input type="file" accept="application/json,.json" onChange={event=>importFile(event.target.files?.[0])}/></label><a className="inline-link" href="/api/export?kind=schema">قاموس البيانات</a></div>{run?.error&&<div className="system-message danger"><AlertTriangle size={17}/>{run.error}</div>}</section>}

function CompanyDialog({selected,strategy,loading,onClose}:{selected:Snapshot|null;strategy:'core'|'bounce';loading:boolean;onClose:()=>void}){const evaluation=selected?evaluateStrategy(strategy,selected):null;return <Dialog open={!!selected} onOpenChange={open=>!open&&onClose()}><DialogContent className="company-dialog" dir="rtl"><DialogTitle>{selected?.symbol} · {selected?.name}</DialogTitle><DialogDescription>{loading?'جارٍ جلب التاريخ وCompany Facts والتحليل العميق…':`تفاصيل الفحص والأدلة — ${strategy==='core'?'Core Value':'Bounce'}`}</DialogDescription>{selected&&evaluation&&<><div className="dialog-summary"><div><small>السعر</small><b>${money(selected.price)}</b></div><div><small>القيمة السوقية</small><b>${money(selected.marketCap)}</b></div><div><small>الدرجة</small><b>{evaluation.score==null?'—':`${evaluation.score.toFixed(1)} / 100`}</b><small>{evaluation.scoreCoverage?`تغطية ${evaluation.scoreCoverage}%`:'بوابات فقط'}</small></div><div><small>جودة البيانات</small><b>{selected.confidence||'F'}</b></div></div><div className="company-description"><h3>ملخص الشركة</h3><p>{selected.description||`شركة مدرجة في ${selected.sector||'السوق الأمريكي'}${selected.industry?` وتعمل في مجال ${selected.industry}`:''}. الوصف التفصيلي يعتمد على ملف الشركة الرسمي عند توفره.`}</p></div><div className="company-financials"><div><small>الإيرادات</small><b>${money(selected.revenue)}</b></div><div><small>صافي الدخل</small><b>${money(selected.netIncome)}</b></div><div><small>التدفق النقدي الحر</small><b>${money(selected.fcf)}</b></div><div><small>EV/S</small><b>{selected.evSales==null?'—':selected.evSales.toFixed(2)}</b></div><div><small>السيولة اليومية</small><b>${money(selected.medianDollarVolume20d)}</b></div><div><small>التخفيف</small><b>{pct(selected.dilution)}</b></div></div><PriceChart snapshot={selected} bounce={strategy==='bounce'}/>{selected.dataIssues?.map(issue=><div className="system-message danger" key={issue}><AlertTriangle size={16}/>{issue}</div>)}{strategy==='core'&&evaluation.factors?.length>0&&<><h3 className="dialog-section-title">تفصيل الدرجة من 100</h3><div className="factor-list">{evaluation.factors.map((factor:any)=><div className="factor-row" key={factor.id}><div><b>{factor.label}</b><small>{factor.available?factor.explanation:'البيانات غير متاحة — لم تُحتسب نقاطًا'}</small></div><strong>{factor.points.toFixed(1)} / {factor.maxPoints}</strong></div>)}</div></>}<h3 className="dialog-section-title">بوابات الاستراتيجية</h3><div className="gate-list">{evaluation.checks.map((check:any)=><div className="gate-row" key={check.id}><span className={`gate-dot ${check.status.toLowerCase()}`}/><div><b>{check.label}</b><small>{check.explanation}</small></div><span className={`result-badge ${check.status.toLowerCase()}`}>{check.status==='PASS'?'اجتاز':check.status==='FAIL'?'فشل':'ناقص'}</span></div>)}</div>{selected.analystTarget!=null&&<div className="analyst-card"><b>متوسط هدف المحللين: ${money(selected.analystTarget)}</b><span>{selected.price&&selected.analystTarget?`${pct(selected.analystTarget/selected.price-1)} فوق السعر الحالي`:'المصدر متاح دون نسبة محسوبة'}</span></div>}{selected.news?.length?<><h3 className="dialog-section-title">آخر الأخبار</h3><div className="news-list">{selected.news.map((item:any)=><a className="news-item" href={item.link||'#'} target="_blank" rel="noreferrer" key={`${item.title}-${item.publishedAt}`}><b>{item.title}</b><small>{item.publishedAt||'تاريخ غير متاح'} · {item.source}</small></a>)}</div></>:null}{strategy==='bounce'&&selected.price&&<div className="trade-plan"><span>خطة الخروج الموثقة</span><b>هدف ${money(selected.price*1.2)}</b><b>وقف ${money(selected.price*.85)}</b><b>3 أشهر كحد أقصى</b></div>}<h3 className="dialog-section-title">مصادر الأرقام</h3><div className="evidence-list">{Object.entries(selected.provenance||{}).map(([key,p]:any)=><div className="evidence-row" key={key}><div><b>{key}</b><small>{p.source}</small></div><span>{p.periodEnd}</span>{p.url?<a href={p.url} target="_blank" rel="noreferrer">فتح المصدر ↗</a>:<span>—</span>}</div>)}</div><div className="research-completeness"><span>اكتمال زوايا البحث</span><b>{Object.values(selected.research||{}).filter(Boolean).length}/4</b><small>لا يدخل السهم ترتيبًا نهائيًا قبل اكتمال المالية، التقييم، المحللين والقطاع.</small></div></>}</DialogContent></Dialog>}
