 ...
 return <div className="terminal-shell" dir="rtl">
  ...
  <main className="app-main">
   ...
   <div className="content">
    <section className="command-header"><div><span className="kicker">مساحة البحث الاستثماري</span><h1>{tabTitles[tab]}</h1><p>{tab==='home'?'فلترة الأسهم الأمريكية الصغيرة بقواعد قابلة للتدقيق.':tab==='lab'?'اختبر القواعد والتكاليف قبل اعتماد أي نتيجة.':tab==='data'?'راقب مصادر كل رقم وجودته وتاريخ توفره.':'نتائج واضحة، أسباب معلنة، وفجوات لا تُخفى.'}</p></div><div className="scan-actions"><button className="scan-primary" onClick={()=>resumable?scan(run.source?.includes('quick')?'quick':'full'):setScanMenu(v=>!v)} disabled={busy}><RefreshCw size={17} className={busy||resumable?'spin':''}/>{busy?'جارٍ البدء':resumable?'الفحص يعمل':'بدء فحص'}</button>{scanMenu&&<div className="scan-popover"><button onClick={()=>scan('quick')}><b>فحص سريع</b><span>12 سهمًا موزعة ضمن نطاق الشركات الصغيرة</span></button><button onClick={()=>scan('full')}><b>فحص السوق الكامل</b><span>تنظيف السوق أولًا ثم تحليل المرشحين فقط</span></button></div>}</div></section>

    {tab==='home'&&!notificationsEnabled&&<section className="notification-panel"><span className="notification-panel-icon"><Bell size={20}/></span><div><b>استلم تنبيهًا عند اكتمال الفحص</b><p>{notificationState==='denied'?'الإشعارات محظورة حاليًا. فعّلها من إعدادات الهاتف ثم افتح التطبيق مجددًا.':notificationState==='unsupported'?'على iPhone ثبّت التطبيق على الشاشة الرئيسية وافتحه من الأيقونة حتى تتوفر الإشعارات.':'اضغط مرة واحدة للسماح بالإشعارات. سنرسل إشعار اختبار فورًا للتأكد أن هاتفك مرتبط.'}</p></div><button onClick={notificationState==='unsupported'?()=>setInstallHelp(true):enableNotifications}>{notificationState==='unsupported'?'طريقة التثبيت':notificationState==='denied'?'فتح التعليمات':'تفعيل واختبار'}</button></section>}
    {tab==='home'&&notificationsEnabled&&<section className="notification-panel enabled"><span className="notification-panel-icon"><Check size={20}/></span><div><b>الإشعارات مفعّلة على هذا الجهاز</b><p>يمكنك إغلاق التطبيق بعد بدء الفحص؛ ستصلك رسالة عند انتهاء الجولة.</p></div><button onClick={enableNotifications}>إعادة الاختبار</button></section>}
    ...
    {run&&<section className="scan-strip"><div className="scan-strip-head"><div><span className={`run-state ${run.status}`}>{run.status==='complete'?'مكتمل':run.status==='failed'?'فشل':resumable?'قيد التنفيذ':'جزئي'}</span><b>{run.source?.includes('quick')?'فحص سريع':'فحص السوق'}</b><small>{stageLabels[run.stage]||`المرحلة ${run.stage}`}</small><small dir="ltr">{new Date(run.created_at).toLocaleString('en-GB')}</small></div><div className="scan-numbers"><span><b>{(run.offset||0).toLocaleString('en-US')}</b> تم تقييمها</span><span><b>{(run.total||0).toLocaleString('en-US')}</b> مرشحة</span>{run.screened_out>0&&<span><b>{run.screened_out.toLocaleString('en-US')}</b> استُبعدت مبكرًا</span>}<span className={run.failed||run.sec_failed?'negative':''}><b>{(run.failed||0)+(run.sec_failed||0)}</b> أخطاء</span></div></div><Progress value={progress}/><div className="progress-caption"><span>{progress.toFixed(1)}%</span><span>{resumable?'الأسعار والأساسيات تُجمع جماعيًا، ثم يُحسب التقييم محليًا مع حفظ نقطة الاستئناف.':'آخر لقطة متاحة للعرض.'}</span></div></section>}
    ...
    {tab==='lab'?<LabPanel lab={lab} setLab={setLab}/>:tab==='data'?<DataPanel run={run} exportData={exportData} importFile={importFile}/>:<>
     <section className="strategy-switch" aria-label="اختيار الاستراتيجية"><button className={strategy==='core'?'active':''} onClick={()=>{setStrategy('core');setFilter('all')}}><span><ShieldCheck size={18}/><b>Core Value</b></span><small>قيمة سوقية $25M–$2B · أفق 12–24 شهرًا</small></button><button className={strategy==='bounce'?'active':''} onClick={()=>{setStrategy('bounce');setFilter('all')}}><span><Activity size={18}/><b>Bounce</b></span><small>هدف +20% · وقف −15% · حتى 3 أشهر</small></button><div className="strategy-note"><Info size={16}/><span>{strategy==='core'?'يعرض المحرك اجتياز البوابات فقط حتى اعتماد الدرجات الفرعية.':'السيولة 150 ألف دولار حد تشغيلي معلن وقيد التحقق البحثي.'}</span></div></section>

     <section className="research-panel"><div className="panel-heading"><div><span className="section-eyebrow">نتائج الاستراتيجية</span><h2>{tab==='favorites'?'قائمة المتابعة':'نتائج الفحص'} <em>{rows.length}</em></h2></div><button className="ghost-button" onClick={exportData}><Download size={16}/> تصدير التدقيق</button></div><div className="control-row"><Tabs value={filter} onValueChange={setFilter} dir="rtl"><TabsList><TabsTrigger value="all">الكل</TabsTrigger><TabsTrigger value="PASS">اجتازت</TabsTrigger><TabsTrigger value="UNKNOWN">تحتاج تحقق</TabsTrigger><TabsTrigger value="FAIL">مستبعدة</TabsTrigger></TabsList></Tabs><label className="search-field"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="اسم الشركة أو الرمز" aria-label="بحث الشركات"/></label></div>
