  ...
  <main className="radar-main">
   ...
   <section className={`strategy-brief ${view}`}>
    {view==='favorites'?<><div className="section-line"><h2>المفضلة ({favorites.length})</h2><button className="text-button" onClick={()=>setView('bounce')}>عودة للكل</button></div><p>قائمتك الخاصة. الحفظ لا يجعل السهم مؤهلًا للاستراتيجية.</p></>:<>
     <p>{view==='bounce'?<>فرص ارتداد قصيرة الأجل نحو <b>+20%</b>. الهبوط، القاع، التخفيف، والانعكاس أعلى MA30W شروط دخول صارمة؛ بعد اجتيازها ترتب الدرجة الأسهم من 100 حسب جودة الفرصة.</>:<>ترتيب قيمة طويلة الأجل يجمع التقييم والربحية والنمو والميزانية. وجود الإيرادات، حد التقييم، والربحية/FCF بوابات دخول صارمة، ثم ترتب الدرجة الشركات حسب جودة العوامل المتاحة.</>}</p>
     ...
     <div className="brief-stats"><div><b>{count??0}</b><span>شركة في الترتيب</span></div><div><b>{view==='core'?data?.summary.coreQualified??0:data?.summary.bounceQualified??0}</b><span>أهلية مؤكدة</span></div><div><b>{view==='core'?data?.summary.coreUnknown??0:data?.summary.bounceUnknown??0}</b><span>أهلية تحتاج تحققًا</span></div></div>
     <details><summary>كيف تُقرأ الدرجة والأهلية؟</summary><p>القائمة تعرض الشركات الموجودة في آخر لقطة مرتبة بالدرجة من الأعلى إلى الأقل. شروط الدخول الموثقة للفئة بوابات صارمة؛ فشل أي منها أو نقص دليله يمنع التأهل ولا تستطيع الدرجة إنقاذه. بعد اجتياز البوابات، تستخدم الدرجة الموزونة لترتيب الشركات. البيانات الناقصة لا تصبح صفرًا أو PASS؛ تقلل التغطية والثقة.</p><p>حالة النموذج الحالية <b>تشخيصية</b>. لا تُفعّل أوزان تنبؤية أو احتمالات قبل dataset تاريخي Point-in-Time يشمل الشركات المشطوبة واختبار نهائي مغلق.</p></details>
    </>}
   </section>
