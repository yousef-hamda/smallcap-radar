  ...
  <main className="radar-main">
   ...
   <section className={`strategy-brief ${view}`}>
    {view==='favorites'?<><div className="section-line"><h2>المفضلة ({favorites.length})</h2><button className="text-button" onClick={()=>setView('bounce')}>عودة للكل</button></div><p>قائمتك الخاصة. الحفظ لا يجعل السهم مؤهلًا للاستراتيجية.</p></>:<>
     <p>{view==='bounce'?<>هدف مختلف عن بقية القوائم: البحث عن ارتداد قصير الأجل نحو <b>+20%</b>. الدرجة من 100 ترتب قوة الهبوط والانعكاس والسيولة والتخفيف، والقبول يبقى مشروطًا باجتياز كل البوابات.</>:<>شركات قوية الأساسيات وسعرها منخفض — فحص التقييم والربحية والسيولة والميزانية. أفق القيمة الأساسية طويل الأجل؛ الدرجة لا تنقذ سهمًا فشل بوابة أساسية.</>}</p>
     ...
     <details><summary>الأدلة الكاملة والمخاطر</summary><p>نعرض الأسهم التي اجتازت كل بوابات القبول فقط. البيانات الناقصة ليست نجاحًا. {view==='bounce'?'درجة الارتداد ترتيب تشخيصي قابل لإعادة الحساب، وليست احتمال ربح؛ معادلة الخادم القديم غير منشورة في الملفات.':'الدرجة التشخيصية قابلة لإعادة الحساب، وليست نموذجًا مثبتًا للتنبؤ بالعائد.'}</p><p>إحصاءات المرجع القديم {view==='bounce'?'65.6% مقابل 43.9% ووسيط 28 يومًا':'‎+23% مقابل +3% وانهيار 25%'} لا تمثل اختبارًا مُعادًا على بيانات هذه النسخة.</p></details>
    </>}
   </section>
