  ...
  <main className="radar-main">
   ...
   <section className={`strategy-brief ${view}`}>
    {view==='favorites'?<><div className="section-line"><h2>المفضلة ({favorites.length})</h2><button className="text-button" onClick={()=>setView('bounce')}>عودة للكل</button></div><p>قائمتك الخاصة. الحفظ لا يجعل السهم مؤهلًا للاستراتيجية.</p></>:<>
     ...
     <div className="brief-stats"><div><b>{count??0}</b><span>شركة في الترتيب</span></div><div><b>{view==='core'?data?.summary.coreQualified??0:data?.summary.bounceQualified??0}</b><span>بوابات سلامة مؤكدة</span></div><div><b>{view==='core'?data?.summary.coreUnknown??0:data?.summary.bounceUnknown??0}</b><span>بيانات تحتاج تحققًا</span></div></div>
    <details><summary>كيف تُقرأ الدرجة والأهلية؟</summary><p>القائمة تعرض الشركات الموجودة في آخر لقطة مرتبة بالدرجة من الأعلى إلى الأقل. بوابات السلامة تتحقق من صلاحية السهم وسلامة المصدر والحداثة فقط. أما شروط الفئة الاقتصادية والسعرية فهي عوامل موزونة؛ فشل عامل يخفض الدرجة ولا يحذف الشركة. البيانات الناقصة لا تصبح صفرًا أو PASS؛ تقلل التغطية والثقة وتظهر كغير مثبتة.</p><p>حالة النموذج الحالية <b>تشخيصية</b>. لا تُفعّل أوزان تنبؤية أو احتمالات قبل dataset تاريخي Point-in-Time يشمل الشركات المشطوبة واختبار نهائي مغلق.</p></details>
    </>}
   </section>
