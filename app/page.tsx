...
type RadarData={run:ScanRun|null;dataRun:ScanRun|null;dataRunId?:string;snapshots:Snapshot[];favorites:string[];summary:{total:number;coreQualified:number;bounceQualified:number;coreRanked?:number;bounceRanked?:number;coreUnknown:number;bounceUnknown:number;coreFailed?:number;bounceFailed?:number;stale?:boolean};page:{hasMore:boolean;offset?:number}};
  ...
  <main className="radar-main">
   ...
   <section className={`strategy-brief ${view}`}>
    {view==='favorites'?<><div className="section-line"><h2>المفضلة ({favorites.length})</h2><button className="text-button" onClick={()=>setView('bounce')}>عودة للكل</button></div><p>قائمتك الخاصة. الحفظ لا يجعل السهم مؤهلًا للاستراتيجية.</p></>:<>
     <p>{view==='bounce'?<>فرص ارتداد قصيرة الأجل نحو <b>+20%</b>. الدرجة من 100 ترتب قوة العوامل الموثقة، بينما حالة PASS/FAIL/UNKNOWN تحدد الأهلية ولا تنقذها الدرجة.</>:<>شركات قوية الأساسيات وسعرها منخفض — فحص التقييم والربحية والسيولة والميزانية. الدرجة من 100 ترتب قوة العوامل المتاحة، والأهلية تبقى منفصلة عن الترتيب.</>}</p>
     <div className="brief-stats"><div><b>{count??0}</b><span>شركة في الترتيب</span></div><div><b>{view==='core'?data?.summary.coreQualified??0:data?.summary.bounceQualified??0}</b><span>اجتازت كل البوابات</span></div><div><b>{view==='core'?data?.summary.coreUnknown??0:data?.summary.bounceUnknown??0}</b><span>بيانات تحتاج تحققًا</span></div></div>
     <details><summary>الأدلة الكاملة والمخاطر</summary><p>القائمة تعرض كل الشركات الموجودة في آخر لقطة مرتبة بالدرجة من الأعلى إلى الأقل. PASS/FAIL/UNKNOWN حالة مستقلة؛ البيانات الناقصة لا تصبح نجاحًا، والسهم الفاشل لا يصبح مؤهلًا بسبب درجة عالية. {view==='bounce'?'درجة الارتداد الحالية ترتيب تشخيصي قابل لإعادة الحساب وليست احتمال ربح؛ الأوزان التنبؤية التاريخية لا تُفعّل قبل اكتمال دراسة PIT.':'الدرجة الحالية ترتيب تشخيصي قابل لإعادة الحساب؛ الأوزان التنبؤية التاريخية لا تُفعّل قبل اكتمال دراسة PIT.'}</p><p>إحصاءات المرجع القديم {view==='bounce'?'65.6% مقابل 43.9% ووسيط 28 يومًا':'‎+23% مقابل +3% وانهيار 25%'} لا تمثل اختبارًا مُعادًا على بيانات هذه النسخة.</p></details>
    </>}
   </section>
   ...
   {loading?<div className="list-loading" role="status"><RefreshCw className="spin"/> جارٍ تحميل البيانات المحفوظة…</div>:evaluated.length?<ul className="stock-list">{evaluated.map(({s,e},index)=><li className={`stock-card ${strategy} ${favorites.includes(s.symbol)?'saved':''}`} key={s.symbol}>
    ...
    <div className="stock-top"><div className="stock-identity"><span className="rank-number" dir="ltr">#{(data?.page.offset??0)+index+1}</span><b dir="ltr">{s.symbol}</b><button className="copy-button" onClick={()=>copy(s.symbol)} aria-label={`نسخ ${s.symbol}`}>{copied===s.symbol?'تم':'نسخ'}</button><button className={`star-button ${favorites.includes(s.symbol)?'saved':''}`} onClick={()=>favorite(s)} disabled={saving===s.symbol} aria-label={`${favorites.includes(s.symbol)?'إزالة':'إضافة'} ${s.symbol} ${favorites.includes(s.symbol)?'من':'إلى'} المفضلة`}><Star size={21} fill={favorites.includes(s.symbol)?'currentColor':'none'}/></button><span dir="auto">{s.name}</span></div><div className="card-score"><strong>{e.score==null?'—':e.score.toFixed(1)}</strong><small>من 100 · {e.status==='PASS'?'مؤهل':e.status==='FAIL'?'فشل بوابة':'غير مثبت'}</small><em className={e.status.toLowerCase()}>{e.status==='PASS'?'PASS':e.status==='FAIL'?'FAIL':'UNKNOWN'} · تغطية {e.scoreCoverage.toFixed(1)}%</em></div></div>
