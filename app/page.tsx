...
function StockTable({rows,strategy,favorites,onSelect,onFavorite}:{rows:any[];strategy:'core'|'bounce';favorites:string[];onSelect:(s:Snapshot)=>void;onFavorite:(symbol:string)=>void}){return <div className="table-wrap"><div className="table-head"><span>الشركة</span><span>السعر</span><span>القيمة السوقية</span><span>12 شهرًا</span><span>الحالة</span><span>جودة البيانات</span><span/></div>{rows.map(({s,e})=>{const screening=(e as any).screeningQualified;const badge=screening&&e.status!=='PASS'?'اجتاز الشروط':labels[e.status];return <div className="table-row" key={s.symbol}><button className="issuer" onClick={()=>onSelect(s)}><span className="ticker">{s.symbol.slice(0,2)}</span><span><b dir="ltr">{s.symbol}</b><small>{s.name}</small></span></button><button className="cell-number" onClick={()=>onSelect(s)}><b>$${money(s.price)}</b></button><button className="cell-number" onClick={()=>onSelect(s)}><b>$${money(s.marketCap)}</b></button><button className={`cell-number ${s.return12m!=null&&(s.return12m<0?'down':'up')}`} onClick={()=>onSelect(s)}><b>{pct(s.return12m)}</b></button><button className="status-cell" onClick={()=>onSelect(s)}><span className={`result-badge ${screening?'pass':e.status.toLowerCase()}`}>{badge}</span><small>{screening&&e.status!=='PASS'?'بانتظار التحقق النهائي':e.status==='PASS'?'كل الشروط مكتملة':e.checks.find((c:any)=>c.status===e.status)?.label||'راجع تفاصيل الفحص'}</small></button><button className="quality-cell" onClick={()=>onSelect(s)}><b>{s.confidence||'F'}</b><small>{strategy==='core'?'الترتيب النهائي معلّق':'بوابات فقط'}</small></button><button className={`watch ${favorites.includes(s.symbol)?'saved':''}`} aria-label="حفظ في قائمة المتابعة" onClick={()=>onFavorite(s.symbol)}><Star size={17}/></button></div>})}</div>}
