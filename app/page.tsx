...
export default function RadarApp(){
 ...
 const resumable=run&&run.source?.includes('Nasdaq/SEC v5')&&['running','partial'].includes(run.status)&&(run.offset<run.total||run.retryPending);
 ...
 return <div className="terminal-shell" dir="rtl">
  ...
  <main className="app-main">
   ...
   <div className="content">
    ...
    {run&&<section className="scan-strip"><div className="scan-strip-head"><div><span className={`run-state ${run.status}`}>{run.status==='complete'?'مكتمل':run.status==='failed'?'فشل':resumable?'قيد التنفيذ':'جزئي'}</span><b>{run.source?.includes('quick')?'فحص سريع':'فحص السوق'}</b><small dir="ltr">{new Date(run.created_at).toLocaleString('en-GB')}</small></div><div className="scan-numbers"><span><b>{(run.offset||0).toLocaleString('en-US')}</b> تم فحصها</span><span><b>{(run.total||0).toLocaleString('en-US')}</b> مرشحة</span>{run.screened_out>0&&<span><b>{run.screened_out.toLocaleString('en-US')}</b> استُبعدت مبكرًا</span>}<span className={run.failed?'negative':''}><b>{run.failed||0}</b> أخطاء</span></div></div><Progress value={progress}/><div className="progress-caption"><span>{progress.toFixed(1)}%</span><span>{resumable?'الفحص الخلفي يعمل مع حفظ نقطة الاستئناف. يمكنك إغلاق التطبيق.':'آخر لقطة متاحة للعرض.'}</span></div></section>}
...
function DataPanel({run,exportData,importFile}:{run:any;exportData:()=>void;importFile:(f?:File)=>void}){return <section className="research-panel content-panel"><div className="panel-heading"><div><span className="section-eyebrow">مصادر البيانات</span><h2>مركز البيانات</h2></div><button className="ghost-button" onClick={exportData}><Download size={16}/> سجل التدقيق</button></div><div className="source-grid"><article><div className="source-head"><span><Wifi size={18}/></span><div><b>Nasdaq</b><small>الدليل · الأسعار · التاريخ · حجم التداول</small></div><span className="source-state"><span className="live-dot"/> أساسي</span></div><p>مصدر السوق الأساسي. توجد نسخة رسمية مضمّنة تتيح بدء الفحص حتى عند تعطل الطلب المباشر.</p></article><article><div className="source-head"><span><Server size={18}/></span><div><b>SEC EDGAR</b><small>Company Facts</small></div><span className="source-state review">مساند</span></div><p>الإيرادات والأرباح والتدفقات النقدية. تعطل SEC لا يوقف فحص الأسعار أو يلغي النتائج المتاحة.</p></article></div><div className="data-actions"><div><h3>استيراد لقطة موثقة</h3><p>JSON من نوع Snapshot[] مع المصدر وتاريخ التوفر لكل مقياس.</p></div><label className="ghost-button upload"><FileUp size={16}/> اختيار ملف<input type="file" accept="application/json,.json" onChange={event=>importFile(event.target.files?.[0])}/></label><a className="inline-link" href="/api/export?kind=schema">قاموس البيانات</a></div>{run?.error&&<div className="system-message danger"><AlertTriangle size={17}/>{run.error}</div>}</section>}
