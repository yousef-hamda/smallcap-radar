  ...
  <main className="radar-main">
   ...
   <section className={`strategy-brief ${view}`}>
    {view==='favorites'?<><div className="section-line"><h2>المفضلة ({favorites.length})</h2><button className="text-button" onClick={()=>setView('bounce')}>عودة للكل</button></div><p>قائمتك الخاصة. الحفظ لا يجعل السهم مؤهلًا للاستراتيجية.</p></>:<>
     <p>{view==='bounce'?<>فرص ارتداد قصيرة الأجل نحو <b>+20%</b>. الهبوط والانعكاس والتخفيف عوامل موزونة في الدرجة، وليست بوابات قاطعة. الأهلية تقتصر على نوع السهم والحجم والسيولة وسلامة البيانات.</>:<>ترتيب قيمة طويلة الأجل يجمع التقييم والربحية والنمو والميزانية. هذه الصفات عوامل موزونة، بينما الأهلية والسلامة منفصلتان عن الدرجة.</>}</p>
     ...
     <details><summary>كيف تُقرأ الدرجة والأهلية؟</summary><p>القائمة تعرض كل الشركات الموجودة في آخر لقطة مرتبة بالدرجة من الأعلى إلى الأقل. فشل عامل مثل الربحية أو شرط الهبوط يخفض الترتيب ولا يحذف السهم. أما فشل بوابة أهلية حقيقية فلا تستطيع الدرجة إنقاذه. البيانات الناقصة لا تصبح صفرًا أو PASS؛ تقلل التغطية والثقة.</p><p>حالة النموذج الحالية <b>تشخيصية</b>. لا تُفعّل أوزان تنبؤية أو احتمالات قبل dataset تاريخي Point-in-Time يشمل الشركات المشطوبة واختبار نهائي مغلق.</p></details>
    </>}
   </section>
